...
module.exports = function registerImpostor(io, socket, impRooms, getAIResponse) {
  ...
  socket.on('impStartGame', async ({ roomCode, category, spyCount }) => {
    ...
    const eligible = room.players.filter(p => !p.spectator && p.name && p.name.length > 1);
    if (eligible.length < actualSpyCount) { socket.emit('error', 'مش فيه لاعبين كفاية للجواسيس'); room.started = false; return; }
